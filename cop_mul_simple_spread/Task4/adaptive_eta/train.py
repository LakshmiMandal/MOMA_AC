...
class B_model():
    ...
    def bert_model(self):
        ...
        prompt = "agent_0 go to landmark 0, agent_1 go to landmark 0, agent_2 go to landmark 0."
...
class A2CAgent:
    def __init__(self, state_size, action_size):
        ...
        self.critic_pt = torch.load(path_C)      
        self.critic1 = Critic(3*(self.state_size)+(26*768), self.value_size, self.critic_lr)
    ...
    def train_model(self, state,log_prob0,log_prob1, log_prob2,sin_stage_reward, next_state, b_hid_st, done,e):
        ...
        if(e<=2000):
            ...
            self.eta=0.6             
        # self.critic=self.eta*self.critic_pt+(1-self.eta)*self.critic1
        value_pt = self.critic_pt(state_tensor_b)[0]
        value1=self.critic1(state_tensor_b)[0]
        value=(self.eta*value_pt)+((1-self.eta)*value1)
        
        # value.requires_grad_(True)
        next_value_pt = self.critic_pt(next_state_tensor_b)[0]
        next_value1 = self.critic1(next_state_tensor_b)[0]
        next_value=self.eta*next_value_pt+(1-self.eta)*next_value1

        if done:
            advantages = (sin_stage_reward - value)
            # advantages1 = (sin_stage_reward - value1)
            # advantages2 = (sin_stage_reward - value)
            target1= sin_stage_reward
        else:
            advantages = (sin_stage_reward + self.discount_factor * (next_value.detach()) - value)
            # advantages1 = (sin_stage_reward + self.discount_factor * (next_value1.detach()) - value1)
            # advantages2 = (sin_stage_reward + self.discount_factor * (next_value.detach()) - value)
            target1= sin_stage_reward + self.discount_factor * (next_value1.detach())
        ...
        #Train Critic
        loss_critic1 = F.mse_loss(value1, target1)
...
if __name__ == "__main__":
    ...
    for runs in range(total_runs):
            if not os.path.exists("./model"+str(runs)):   
                os.makedirs("./model"+str(runs))
            actor0_save_path="model"+str(runs)+"/actor0.pkl"
            actor1_save_path="model"+str(runs)+"/actor1.pkl"
            actor2_save_path="model"+str(runs)+"/actor2.pkl" 
            critic1_save_path="model"+str(runs)+"/critic1.pkl"
            f=open("simple_spread_logFile_itr_avgCost"+str(runs),"w+")
        ...
        # for i in range(1):
            ...
            for e in range(EPISODES):
                ...
                while (env.agents):
                    ...
                    # else:
                    #     done=False    
                    agent.train_model(full_state,log_prob0,log_prob1,log_prob2,reward_single_stage,next_full_state, hid_st,done,e)
                ...
                #     print("value:::",agent.critic_pt(state_tensor_b)[0])
                #     plot1 = plt.figure(1)
                ...
                if(e%100==0):
                    ...
                    torch.save(agent.critic1, critic1_save_path)    
