...
class A2CAgent:
    def __init__(self, no_agent,state_size, action_size):
        ...
        self.actor0 =torch.load(actor0_save_path)
        self.actor1 =torch.load(actor1_save_path)
        self.actor2 =torch.load(actor2_save_path)
        path_C = "./model/critic.pkl"
        self.critic_pt = torch.load(path_C)
        self.critic = torch.load(path_C)
        self.critic1 =torch.load(critic1_save_path)
...
# env.render()
    ...
    print("average reward, std",np.mean(episodic_rewards),np.std(episodic_rewards))  